...
Class.Mixin(Screen, {
	...
	buildChild: function(container, child_params) {
		...
		if (child_params.type === 'sprite') {
		...
		} else if (child_params.type === 'gif') {

			child = App.resources[child_params.image].animation;
			...
			child.play = function(time, next) {
				...
				if (asset && asset["jsmpeg"] && App.Assets[video + ':sound']) child.sound = App.Assets[video + ':sound'].sound;
			...
			if (spine_atlas_asset.type === 'text') {
				...
				App.Assets[child_params.spineData].data.skeleton.spine = '3.8.99';
	...
	applyParamFilter: function(sprite, filters_params) {
		...
		this.each(filters_params, function(filter_params) {
			...
			let params = (filter_params[1] || []).concat([]);

			params.splice(0, 0, null);

			let filter = new (Function.prototype.bind.apply(PIXI.filters[filter_params[0]], params));
